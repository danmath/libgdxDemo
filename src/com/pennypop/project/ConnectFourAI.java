...
public class ConnectFourAI {
	public static int[] getMoveScore(ConnectFourBoard gameBoard, int depth) {
		...
		return minimax(gameBoarded, depth);
	}
	
	public static int[] minimax(ConnectFourBoard gameBoard, int depth) {
		int[] ans = new int[gameBoard.getWidth()];
		...
		for(int i = 0; i < gameBoard.getWidth(); i++) {
			...
			if(gameBoard.makeMove(i)) {
				...
				if(yellowsTurn) {
					ans[i] = min(gameBoard, depth-1);
				}
				else {
					ans[i] = max(gameBoard, depth-1);
				}
	...
	//red's turn
	public static int min(ConnectFourBoard gameBoard, int depth) {
		...
		if(curState != GameState.NOT_OVER) {
			...
			for(int i = 0; i < gameBoard.getWidth(); i++) {
				if(gameBoard.makeMove(i)) {
					int curScore = max(gameBoard, depth-1);
	...
	//yellow's turn
	public static int max(ConnectFourBoard gameBoard, int depth) {
		System.out.println("state: " + gameBoard.boardStateString());
		int score = 0;
		GameState curState = gameBoard.getGameState();
		if(curState != GameState.NOT_OVER) {
			...
			for(int i = 0; i < gameBoard.getWidth(); i++) {
				if(gameBoard.makeMove(i)) {
					int curScore = min(gameBoard, depth-1);
